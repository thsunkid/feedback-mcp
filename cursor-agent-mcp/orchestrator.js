...
import { existsSync, readFileSync, writeFileSync, readdirSync, statSync, mkdirSync, unlinkSync, createWriteStream } from 'node:fs';
...
function loadState() {
  ...
  catch { return { pid: null, taskId: null, answeredCount: 0, startedAt: null }; }
}
...
function writeAnswer(turnNum, reply) {
  const answerFile = join(SESSION_DIR, `answer_${turnNum}.json`);
  writeFileSync(answerFile, JSON.stringify({ reply, timestamp: Date.now() }), 'utf8');
}

function countAnswered() {
  return readdirSync(SESSION_DIR)
    .filter(f => f.startsWith('answer_') && f.endsWith('.json'))
    .length;
}
...
if (cmd === 'spawn') {
  ...
  if (!prompt) { console.error('Usage: spawn "prompt" [--model M] [--cwd DIR]'); process.exit(1); }

  const modelIdx = process.argv.indexOf('--model');
  const model = modelIdx >= 0 ? process.argv[modelIdx + 1] : 'composer-1';
  ...
  const child = spawnProc('cursor-agent', [
  ...
  console.log(JSON.stringify({ status: 'spawned', pid: child.pid, model, session_dir: SESSION_DIR }));

} else if (cmd === 'check') {
  const q = findPendingQuestion();
  if (q) {
    console.log(JSON.stringify({ pending: true, turn: q.turn, message: q.message }));
  } else {
  ...
  if (!reply) { console.error('Usage: reply "answer text"'); process.exit(1); }
  ...
  console.log(JSON.stringify({ answered: q.turn, total_answered: state.answeredCount }));
  ...
  const state = loadState();
  if (state.pid) {
    try { process.kill(state.pid, 'SIGTERM'); console.log(JSON.stringify({ killed: state.pid })); }
    catch { console.log(JSON.stringify({ error: 'process not found', pid: state.pid })); }
  } else {
    console.log(JSON.stringify({ error: 'no agent running' }));
  }

} else {
  console.log(`Usage: node orchestrator.js <spawn|check|reply|status|result|kill>`);
  process.exit(1);
}
